...
export default function RegisterPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      toast.error(msg.includes('email-already-in-use') ? 'Email already in use' : 'Registration failed');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div style={{ height: '100vh', background: FG.bg0, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ width: '100%', maxWidth: 400, padding: 24, position: 'relative' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <p style={{ color: FG.mid, fontSize: 14, lineHeight: 1.5, marginTop: 8 }}>Built for lifters who do it clean.</p>
        </div>
        ...
        <div style={{ background: FG.bg1, border: `1px solid ${FG.line}`, borderRadius: 20, padding: 24 }}>
          ...
          <FButton variant="ghost" fullWidth onClick={async () => { setLoading(true); try { await loginWithGoogle(); router.replace('/dashboard'); } catch { toast.error('Google sign-in failed'); } finally { setLoading(false); } }}>
